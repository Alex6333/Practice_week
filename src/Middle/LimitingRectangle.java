...
public class LimitingRectangle {
    ...
    int min(int j) {
        ...
        for (int i = 0; i < points.length; i++) {
            if (points[i][j] <= min) min = points[i][j];
        }
        return min;
    }
...
//        конструктор, принимающий двумерный массив целых чисел
//        (массив точек на плоскости вида [[1, 0], [2, 3], ...], 0 элемент — координата по оси X, 1 элемент — по оси Y);
...
//        метод getBorders, возвращает строку вида
//        {нижняя точка по Y, верхняя точка по Y, левая точка по Х, правая точка по Х}.
...
//          В приходящем массиве точек [[1, 0], [2, 3], ...]
//          в скобках лежат просто отдельные координаты для X и для Y, а не координаты одной точки (по X и Y).
